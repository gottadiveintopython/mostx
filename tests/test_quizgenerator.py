...
def test_wrong_argument():
    ...
    with pytest.raises(ValueError):
        q = qgen.generate(choices='AB', n_adjs=0)
    with pytest.raises(ValueError):
        q = qgen.generate(choices='AB', n_adjs=-1)
    with pytest.raises(ValueError):
        q = qgen.generate(choices='AB', n_adjs=qgen.max_adjs + 1)
    with pytest.raises(ValueError):
        q = qgen.generate(choices='', n_adjs=1)
    with pytest.raises(ValueError):
        q = qgen.generate(choices='A', n_adjs=1)
...
def test_one_adjective():
    ...
    q = qgen.generate(choices=range(3), n_adjs=1)
...
def test_two_adjectives():
    ...
    q = qgen.generate(choices='AB', n_adjs=2)
