...
def test_wrong_argument():
    ...
    with pytest.raises(ValueError):
        q = qgen.gen_mostx_quiz(choices='AB', n_adjs=0)
    with pytest.raises(ValueError):
        q = qgen.gen_mostx_quiz(choices='AB', n_adjs=-1)
    with pytest.raises(ValueError):
        q = qgen.gen_mostx_quiz(choices='AB', n_adjs=qgen.max_adjs + 1)
    with pytest.raises(ValueError):
        q = qgen.gen_mostx_quiz(choices='', n_adjs=1)
    with pytest.raises(ValueError):
        q = qgen.gen_mostx_quiz(choices='A', n_adjs=1)
